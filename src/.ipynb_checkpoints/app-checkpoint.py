...
if "token" not in st.session_state:
    print("token not in st.session_state")
    ...
    try:
        ...
        if result and "token" in result:
            ...
            print(f'idc_jwt_token: {st.session_state["idc_jwt_token"]}')
            st.session_state["idc_jwt_token"]["expires_at"] = datetime.now(tz=UTC) + \
                timedelta(seconds=st.session_state["idc_jwt_token"]["expiresIn"])
            print(f'idc_jwt_token expires at: {st.session_state["idc_jwt_token"]["expiresIn"]}')
            st.rerun()
    except Exception as e:
        st.error(f"An error occurred: {e}")
else:
    token = st.session_state["token"]
    print(f'token in session state: {token}')
